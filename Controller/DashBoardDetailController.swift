...
class DashBoardDetailController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1{
            ...
            if mark < 50{
                cell.setupCellView(grade: "N")
            }
            if mark >= 50 && mark < 60{
                cell.setupCellView(grade: "P")
            }
            if mark >= 60 && mark < 70{
...
//                }
    ...
    func setupSectionThreeCell(tableView: UITableView, indexPath: IndexPath, cellTopLeftLabel: String, progress: Float, identifier: String) -> (NutrientProgressCell){
        ...
        if progress > 0.33 && progress <= 0.66{
            cell.middleKeyLabel.text = "Mid"
        }
        if progress > 0.66 && progress <= 1{
            cell.middleKeyLabel.text = "Perfect"
        }
        if progress > 1{
            cell.middleKeyLabel.text = "Overintake"
